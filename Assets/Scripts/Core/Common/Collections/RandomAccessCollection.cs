...
public interface IWeightedElement<T>
{
    int Weight { get; }
    T Value { get; }
}

public class RandomAccessCollection<T> : IRandomAccessCollection<T>
{
    public int Count => values.Count;

    private readonly VoseAliasIndexer indexer;
    private readonly IReadOnlyList<T> values;

    public RandomAccessCollection(IReadOnlyCollection<(int Weight, T Value)> values)
    {
        this.indexer = new VoseAliasIndexer(values.Select(x => x.Weight), maxValue => UnityEngine.Random.Range(0, maxValue));
        this.values = values.Select(x => x.Value).ToArray();
    }

    public RandomAccessCollection(IReadOnlyCollection<IWeightedElement<T>> values)
    {
        this.indexer = new VoseAliasIndexer(values.Select(x => x.Weight), maxValue => UnityEngine.Random.Range(0, maxValue));
        this.values = values.Select(x => x.Value).ToArray();
    }
    ...
    public T NextValue()
    {
        var index = indexer.NextValue();
        return values[index];
    }

    public int NextIndex()
    {
        return indexer.NextValue();
    }
    ...
    private class VoseAliasIndexer
    {
        public delegate int IRandomNumberGenerator(int maxValue);
        ...
        public VoseAliasIndexer(IEnumerable<int> probabilities, IRandomNumberGenerator randomNumberGenerator)
        {
        ...
        {
            var i = _randomNumberGenerator(Probabilities);
            if (_even || _randomNumberGenerator((int)_probabilitySum) < _probabilities[i])
                return i;
            else
                return _alias[i];
        }
